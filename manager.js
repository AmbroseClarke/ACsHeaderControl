...
function appendRuleItem(container, initialData) {
  const { id, conditionType, conditionValue, caseSensitive, refererValue } = initialData;
  ...
  // Set the refererValue input
  ruleItem.querySelector('.referer-value').value = refererValue;
  ...
  ruleItem.querySelector('.save-rule').addEventListener('click', async () => {
    const { id, conditionValue, conditionType, caseSensitive, refererValue } =
...
function getCurrentRuleValues(ruleItem) {
  ...
  const refererValue = ruleItem.querySelector('.referer-value').value.trim();
  return {
    id,
    conditionValue,
    conditionType,
...
async function verify(ruleItem) {
  const { conditionValue, conditionType, caseSensitive, refererValue } =
    getCurrentRuleValues(ruleItem);
...
function renderRules(rules) {
  ...
  for (const rule of rules) {
    // The condition can only be either url-filter or regex-filter.
	...
	//these are the rules read from Chrome
    appendRuleItem(rulesList, {
      id: rule.id,
      conditionType,
      caseSensitive,
      conditionValue: rule.condition[conditionType],
	  refererValue: rule.action.requestHeaders[0].value
    });
  }
}

async function saveRule({ id, conditionType, conditionValue, caseSensitive, refererValue }) {
